...
import com.jingtian.mobileguardian.db.dao.VirusKillerDAO;
...
public class VirusKillerActivity extends Activity {
		...
		public void handleMessage(android.os.Message msg) {
			...
			case SCANNING:
				...
				if (scanInfo.isVirus) {
					...
					tv.setText("Save File: "+scanInfo.appName);
				}
			...
			case FINISH:
				...
				if(virusInfos.size()>0){//Virus found
					AlertDialog.Builder builder = new Builder(VirusKillerActivity.this);
					...
					Toast.makeText(getApplicationContext(), "Your phone is safe!", 0).show();
				}
				break;
			default:
				break;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Toast.makeText(getApplicationContext(), "Virus Scanning processes in backend, feel free to leave this page", 1).show();
	}
...
//		final Message msg = Message.obtain(); //WRONG! one message only serve one time
		virusInfos = new ArrayList<VirusKillerActivity.ScanInfo>();
